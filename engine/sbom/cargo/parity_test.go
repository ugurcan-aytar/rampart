...
// TestParserParity diffs Go and Rust cargo parser outputs byte-for-byte.
func TestParserParity(t *testing.T) {
	...
	for _, name := range fixtures {
		t.Run(name, func(t *testing.T) {
			...
			content, err := os.ReadFile(filepath.Join("../../testdata/lockfiles/cargo", name))
			require.NoError(t, err)

			goParsed, err := cargo.NewParser().Parse(ctx, content)
			require.NoError(t, err, "go parse failed")

			rustParsed, err := client.ParseCargo(ctx, content)
...
// TestParserParity_Errors — both sides surface the same error class on
...
func TestParserParity_Errors(t *testing.T) {
	...
	malformed := []byte("[[package]\nname = ")

	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()

	_, goErr := cargo.NewParser().Parse(ctx, malformed)
	require.Error(t, goErr)
	require.True(t, errors.Is(goErr, cargo.ErrMalformedLockfile))

	_, rustErr := client.ParseCargo(ctx, malformed)
	require.Error(t, rustErr)
	require.True(t, errors.Is(rustErr, native.ErrMalformedLockfile))
}

// --- helpers (mirror npm/parity_test.go) ---------------------------------
...
func startNativeServer(t *testing.T) *serverHandle {
	...
	socketDir, err := os.MkdirTemp("", "rampart-parity-cargo-*")
	require.NoError(t, err)
